<template>
    <div class="main">
        ...
        <div class="contentLeft">
            <div class="topButton">
                <el-select v-model="arId" placeholder="省份" size="mini" class="dishi" clearable @change="selectValue">
                    <el-option v-for="item in city" :key="item.arId" :value="item.arId" :label="item.admRegName"></el-option>
                </el-select>
                <el-select v-model="arId1" placeholder="地市" size="mini" clearable class="dishi" style="margin-left: 4px" @change="selectCityValue">
                    <el-option v-for="item in proCity" :key="item.arId" :value="item.arId" :label="item.admRegName"></el-option>
                </el-select>
                <el-select v-show="areaShow" v-model="arId2" placeholder="区县" size="mini" clearable class="dishi" style="margin-left: 4px" @change="selectArea">
                    <el-option v-for="item in cityArea" :key="item.arId" :value="item.arId" :label="item.admRegName"></el-option>
                </el-select>
                <el-select v-show="areaShow" v-model="searchZsType" placeholder="制式" size="mini" clearable class="dishi" style="margin-left: 4px" @change="searchZsValue">
                    <el-option v-for="item in cellList" :key="item" :label="item" :value="item"></el-option>
                </el-select>
                <!-- <el-select v-model="stanSysType" clearable placeholder="制式" size="mini" class="zhishi" @blur="selectBlurzhishi" @change="selectZsValue">
                    <el-option v-for="item in findStandardSystemList" :key="item.ssId" :label="item.stanSysType" :value="item.stanSysType"></el-option>
                </el-select> -->
                <el-input v-model="input" placeholder="区域名称" size="mini" class="dishi" style="margin-left: 4px"></el-input>
                <el-select v-model="taskStateType" placeholder="状态" size="mini" class="dishi" style="margin-left: 4px" clearable @change="selectStatusValue">
                    <el-option v-for="item in statusList" :key="item.taskStateType" :label="item.taskStateName" :value="item.taskStateType"></el-option>
                </el-select>
                <el-button type="primary" size="mini" @click="seachButton" v-has="'query'" style="float: left; margin-left: 4px">查询</el-button>
          ...
          size="mini"
          class="zhishi"
          @change="selectZsValue"
        >
          <el-option
            v-for="item in findStandardSystemList"
            :key="item.ssId"
            :label="item.stanSysType"
            :value="item.stanSysType"
          ></el-option>
        </el-select> -->

                <!-- <el-select
          v-model="stanSysType1"
          multiple
          collapse-tags
          clearable
          placeholder="频段"
        ...
        >
          ...
          ></el-option>
                ...
                <el-dropdown trigger="click" :hide-on-click="false" class="qxq2" ref="dropdownMenuCellTypePreq">
                    <el-button size="mini" style="color: rgb(202, 203, 210);margin-left: 10px;">
                        选择制式与频段
                    ...
                    <el-dropdown-menu slot="dropdown">
                        <el-dropdown-item disabled>
                            <el-select v-model="stanSysType" clearable placeholder="制式" size="mini" class="zhishi" @blur="selectBlurzhishi" @change="selectZsValue">
                                <el-option v-for="item in selctList" :key="item.ssId" :label="item.stanSysType" :value="item.stanSysType"></el-option>
                            </el-select>
                            <el-select v-model="stanSysType1" multiple collapse-tags clearable placeholder="频段" size="mini" class="zhishi" @change="selectZsValue1">
                                <el-option v-for="item in freqList" :key="item.ssId" :label="item.stanSysType" :value="item.stanSysType"></el-option>
                            </el-select>
                        </el-dropdown-item>
                        <el-dropdown-item style="display: flex; padding-bottom: 3px">
                            <el-tag class="zhishi">选定的制式</el-tag>
                            <el-tag type="info " class="zhishi" style="margin-left: 3px">选定的频段</el-tag>
                        </el-dropdown-item>
                        <el-dropdown-item v-for="(item, index) in this.stanSysTypeFreqBand" :index="index" :key="item.cellType" style="display: flex; padding-bottom: 3px">
                            <el-tag :key="item.cellType" closable class="zhishi" @close="handleClosecellTypeTag(item.cellType)">{{ item.cellType }}</el-tag>
                            <!-- <el-tag type="info" v-for='(ite,ind) in item.freq.split(",")' :key="ind" >{{item.freq}}</el-tag> -->
                            <el-tag type="info pull-right" style="margin-left: 3px; min-width: 110px">{{ item.freq }}</el-tag>
                        </el-dropdown-item>
                    </el-dropdown-menu>
                </el-dropdown>
                <el-select v-model="modelName" placeholder="仿真模型" size="mini" style="width: 100px; margin-left: 11px;" @change="selectModelType">
                    <el-option v-for="(item, index) in modelList"  :index="index" :key="item.modelId" :label="item.modelId < 0 ? item.modelName : `LiShuttleModel_${item.modelId}`" :value="item.modelName"></el-option>
                </el-select>
                <el-select v-model="binWidth" clearable placeholder="精度" size="mini" style="width: 60px; margin-left: 1px" @change="selectBinWidth">
                ...
                <!-- <el-select v-model="antennaEntity" clearable placeholder="天线型号" size="mini" style="width: 95px; margin-left: 1px" @change="selectAntennaEntity">
                    <el-option v-for="(item, index) in rayList" :key="index" :label="item" :value="item"></el-option>
                </el-select> -->
                <el-dropdown trigger="click" :hide-on-click="false" class="qxq">
                    <el-button size="mini" style="color: rgb(202, 203, 210)">
                        场景
                        <i class="el-icon-arrow-down el-icon--right"></i>
                    </el-button>
                    <el-dropdown-menu slot="dropdown">
                        <el-dropdown-item v-for="(item, index) in secenList" :key="index">
                            <el-checkbox v-if="item.secenId == 1" v-model="secenechecked" :label="item.secenId" @change="selectScene(item.secenId)">{{ item.secenType }}</el-checkbox>
                            <el-checkbox v-else :label="item.secenId" @change="selectScene(item.secenId)">{{ item.secenType }}</el-checkbox>
                        </el-dropdown-item>
                    </el-dropdown-menu>
                </el-dropdown>
                <el-dropdown v-if="this.secenList1.includes('4') === true" trigger="click" :hide-on-click="false" class="qxq">
                    <el-button size="mini" style="color: rgb(202, 203, 210)">
                        级别
                        <i class="el-icon-arrow-down el-icon--right"></i>
                    </el-button>
                    <el-dropdown-menu slot="dropdown">
                        <el-dropdown-item v-for="(item, index) in rodeList" :key="index">
                            <el-checkbox :label="item.rodeId" @change="selectLevel(item.rodeId)">{{ item.rodeType }}</el-checkbox>
                        </el-dropdown-item>
                    </el-dropdown-menu>
                </el-dropdown>
                <el-dropdown trigger="click" :hide-on-click="false" class="qxq2">
                    <el-button size="mini" style="color: rgb(202, 203, 210)">
                        其他配置
                        <i class="el-icon-arrow-down el-icon--right"></i>
                    </el-button>
                    <el-dropdown-menu slot="dropdown">
                        <el-dropdown-item style="display: flex">
                            <span style="font-size: 12px">最大高度</span>
                            <el-input v-model="maxHeight" @input="e => (maxHeight = isNumber(e))" placeholder="请输入" size="mini" style="width: 100px; margin-left: 4px"></el-input>
                        </el-dropdown-item>
                        <el-dropdown-item style="display: flex">
                            <span style="font-size: 12px">楼层高度</span>
                            <el-input v-model="floorHeight" @input="e => (floorHeight = isNumber(e))" placeholder="请输入" size="mini" style="width: 100px; margin-left: 4px"></el-input>
                        </el-dropdown-item>
                        <el-dropdown-item style="display: flex">
                            <span style="font-size: 12px">高层层高</span>
                            <el-input v-model="highLevelFloorHeight" @input="e => (highLevelFloorHeight = isNumber(e))" placeholder="请输入" size="mini" style="width: 100px; margin-left: 4px"></el-input>
                        </el-dropdown-item>
                        <el-dropdown-item style="display: flex">
                            <span style="font-size: 12px">高层门限</span>
                            <el-input v-model="highLevelThreadshold" @input="e => (highLevelThreadshold = isNumber(e))" placeholder="请输入" size="mini" style="width: 100px; margin-left: 4px"></el-input>
                        </el-dropdown-item>
                        <el-dropdown-item style="display: flex">
                            <span style="font-size: 12px">干扰抑制</span>
                            <el-input v-model="interFereCancelFig" placeholder="请输入" size="mini" style="width: 100px; margin-left: 4px"></el-input>
                        </el-dropdown-item>
                        <el-dropdown-item style="display: flex">
                            <span style="font-size: 12px">计算半径</span>
                            <el-input v-model="cellCalculationRadius" @input="e => (cellCalculationRadius = isCellCalculationRadiusNum(e))" size="mini" style="width: 100px; margin-left: 5px"></el-input>
                        </el-dropdown-item>
                        <el-dropdown-item style="display: flex">
                            <span style="font-size: 12px">外扩半径</span>
                            <el-input v-model="extendSpread" @input="e => (extendSpread = isExtendSpreadNumber(e))" placeholder="请输入整数(0-5000)" size="mini" style="width: 100px; margin-left: 5px"></el-input>
                        </el-dropdown-item>
                        <el-dropdown-item style="display: flex">
                            <span style="font-size: 12px">引擎参数</span>
                            <el-button size="mini" plain style="width: 100px; margin: 4px" @click="engineParamCfgDialog">计算设置</el-button>
                        </el-dropdown-item>
                        <el-tag type="warning" style="margin-left: 4px">计算半径含2位小数正数=>10</el-tag><br />
                        <el-tag type="warning" style="margin-left: 4px">外扩半径为0-5000之间的整数</el-tag>
                    </el-dropdown-menu>
                </el-dropdown>
                <el-dropdown>
                    <el-button id="button-simulation" v-has="'simulation'" type="primary" size="mini" style="color: rgb(202, 203, 210); margin-top: 0px !important">仿真<i class="el-icon-arrow-down el-icon--right"></i> </el-button>

                    <el-dropdown-menu slot="dropdown">
                        <el-dropdown-item><el-button type="info" plain size="mini" @click="reImg(false)" style="margin-left: 0px">即时仿真</el-button> </el-dropdown-item>
                        <el-dropdown-item><el-button type="info" plain size="mini" @click="reImg(true)" style="margin-left: 0px">快速仿真</el-button> </el-dropdown-item>
                        <el-dropdown-item>
                            <el-button type="info" plain size="mini" @click="timeReImg" style="margin-left: 0px; box-sizing: border-box; margin-top: 3px">定时仿真</el-button>
                        </el-dropdown-item>
                    </el-dropdown-menu>
                </el-dropdown>
                <!-- <el-button type="info" plain size="mini" @click="reImg(false)" style="margin-left: 1px">即时仿真</el-button>  -->

                <!-- fast Simulation -->
                <!-- <el-button type="info" plain size="mini" @click="reImg(true)" style="margin-left: 1px">快速仿真</el-button>         -->
                <el-button type="info" plain size="mini" @click="deleteTable" v-has="'delete'" style="margin-left: 1px">删除</el-button>
                <el-button type="info" plain size="mini" v-has="'dispaly'" @click="showTable" style="margin-left: 0px">显示</el-button>
                <!-- <el-button
          type="info"
          plain
          size="mini"
          @click="timeReImg"
          style="margin-left: 0px;box-sizing:border-box;margin-top: 3px;"
        >定时仿真</el-button> -->
                <el-button type="info" plain size="mini" @click="statistics" v-has="'statisticsSet'" style="margin-left: 0px; box-sizing: border-box;">统计设置</el-button>
                <el-dropdown style="margin-left:5px;">
                <el-button type="primary" size="mini" plain  v-show="!isShowLineArea" v-has="'import'">导出</el-button>
                <el-button type="primary" size="mini" plain  @click="toNokia">跳转</el-button>
                <el-dropdown-menu slot="dropdown">
                        <el-dropdown-item><el-button type="info" plain size="mini" @click="outDown" style="margin-left: 0px">区域统计结果</el-button> </el-dropdown-item>
                        <el-dropdown-item><el-button type="info" plain size="mini" @click="buildingPortraitResults" style="margin-left: 0px">楼宇画像结果</el-button> </el-dropdown-item>
                        <el-dropdown-item><el-button type="info" plain size="mini" @click="downloadExcel" style="margin-left: 0px">任务规划工参</el-button> </el-dropdown-item>
                    </el-dropdown-menu>
                </el-dropdown>
            </div>
            ...
            <div class="content" id="content">
                <div class="left" id="left">
                    <div class="leftTable">
                        <el-dialog title="编辑区域信息" :visible.sync="nameVisilble" :show-close="false" width="30%">
                            <el-form :model="dataList" class="createStyle" ref="dataList" label-width="80px">
                                <el-form-item label="区域名称" prop="pcId">
                                    <el-input v-model="dataList.polygonName" style="width: 93%" size="mini"></el-input>
                                </el-form-item>
                                <div>
                                    <el-form-item label="省市">
                                        <el-select v-model="dataList.province" placeholder="全国" size="mini" class="dishi" clearable @change="selectValueTable" style="width: 30%">
                                            <el-option v-for="item in city" :key="`${item.arId}`" :value="`${item.arId}`" :label="item.admRegName"></el-option>
                                        </el-select>
                                        <el-select v-model="dataList.city" placeholder="地市" size="mini" clearable class="dishi" style="margin-left: 10px; width: 30%" @change="selectValueCity">
                                            <el-option v-for="item in proCity" :key="`${item.arId}`" :value="`${item.arId}`" :label="item.admRegName"></el-option>
                                        </el-select>
                                        <el-select v-model="dataList.townCode" placeholder="区县" size="mini" clearable class="dishi" style="margin-left: 1%;width: 30%" >
                                            <el-option v-for="item in cityArea" :key="`${item.arId}`" :value="`${item.arId}`" :label="item.admRegName"></el-option>
                                        </el-select>
                                    </el-form-item>
                                </div>
                                <el-form-item label="制式">
                                    <el-select
                                        v-model="dataList.cellType"
                                        placeholder="请选择制式"
                                        size="mini"
                                        class="dishi"
                                        clearable
                                        @change="changeEditCell"
                                        style="width: 50%"
                                    >
                                        <el-option v-for="item in cellList" :key="item" :label="item" :value="item"></el-option>
                                    </el-select>
                            ...
                            <div slot="footer" class="dialogFoot">
                                <el-button @click="nameClose('dataList')" size="mini">取 消</el-button>
                                <el-button type="primary" @click="nameSave('dataList')" size="mini">确 定</el-button>
                            </div>
                        </el-dialog>
                        <el-table
                        ...
                        >
                            ...
                            <el-table-column label="归属" width="70" show-overflow-tooltip>
                                <template slot-scope="scope">
                                    <span v-if="scope.row.city && scope.row.taskStatus == '0'" style="color: #ffa500">{{ scope.row.cityName }}</span>
                                    <span v-if="scope.row.city && scope.row.taskStatus == '1'" style="color: blue">{{ scope.row.cityName }}</span>
                                    <span v-if="scope.row.city && scope.row.taskStatus == '2'" style="color: green">{{ scope.row.cityName }}</span>
                                    <span v-if="scope.row.city && scope.row.taskStatus == '3'" style="color: red">{{ scope.row.cityName }}</span>
                                </template>
                            </el-table-column>
                            <el-table-column prop="polygonName" label="区域名称" width="130" show-overflow-tooltip>
                                <!-- 橙色：未开始==未完成；蓝色：仿真中；绿色：仿真完成；红色：仿真失败。 之前绿色是仿真中蓝色是仿真完成-->
                                <template slot-scope="scope">
                                    <span v-if="scope.row.taskStatus == '0'" style="color: #ffa500">
                                        <i class="el-icon-edit" @click="dialogNameVisible(scope.$index, scope.row)"></i>
                                        &nbsp;
                                        {{ scope.row.polygonName }}
                                    </span>
                                    <span v-if="scope.row.taskStatus == '1'" style="color: blue">
                                        <i class="el-icon-edit" @click="dialogNameVisible(scope.$index, scope.row)"></i>
                                        &nbsp;
                                        {{ scope.row.polygonName }}
                                    </span>
                                    <span v-if="scope.row.taskStatus == '2'" style="color: green">
                                        <i class="el-icon-edit" @click="dialogNameVisible(scope.$index, scope.row)"></i>
                                        &nbsp;
                                        {{ scope.row.polygonName }}
                                    </span>
                                    <span v-if="scope.row.taskStatus == '3'" style="color: red">
                                        <i class="el-icon-edit" @click="dialogNameVisible(scope.$index, scope.row)"></i>
                                        &nbsp;
                                        {{ scope.row.polygonName }}
                                    </span>
                            ...
                            <el-table-column prop="polygonCode" label="区域编号" width="110" show-overflow-tooltip>
                                <template slot-scope="scope">
                                    <span v-if="scope.row.taskStatus == '0'" style="color: #ffa500">{{ scope.row.polygonCode }}</span>
                                    <span v-if="scope.row.taskStatus == '1'" style="color: blue">{{ scope.row.polygonCode }} </span>
                                    <span v-if="scope.row.taskStatus == '2'" style="color: green">{{ scope.row.polygonCode }} </span>
                                    <span v-if="scope.row.taskStatus == '3'" style="color: red">{{ scope.row.polygonCode }} </span>
                                </template>
                            </el-table-column>
                        </el-table>
                        <div class="fenye">
                            <Page :current="currentPage" :total="total" simple show-total :page-size="pageSize" @on-change="current_change"></Page>
                            <p>共{{ total }}条</p>
                        </div>
                    </div>
                </div>
                <el-dialog title="多边形绘制" :visible.sync="centerDialogVisible" :close-on-click-modal="false" :show-close="false" width="35%" center>
                    <el-form :model="dbList" class="createStyle" ref="dbList" label-width="80px" :rules="rulesCreate">
                        <el-form-item label="区域名称" prop="polygonName">
                            <el-input v-model="dbList.polygonName" style="width: 93%" size="mini"></el-input>
                        </el-form-item>
                        <div>
                            <el-form-item label="省市">
                                <el-select v-model="dbList.province" placeholder="全国" size="mini" class="dishi" clearable @change="dbselectValueTable" style="width: 30%">
                                    <el-option v-for="item in city" :key="`${item.arId}`" :value="`${item.arId}`" :label="item.admRegName"></el-option>
                                </el-select>
                                <el-select v-model="dbList.city" placeholder="地市" size="mini" clearable class="dishi" style="margin-left: 10px; width: 30%" @change="selectCityValue">
                                    <el-option v-for="item in proCity" :key="`${item.arId}`" :value="`${item.arId}`" :label="item.admRegName"></el-option>
                                </el-select>
                                <el-select v-model="dbList.cityArea" placeholder="区县" size="mini" clearable class="dishi" style="margin-left: 1%;width: 30%" @change="selectArea">
                                    <el-option v-for="item in cityArea" :key="`${item.arId}`" :value="`${item.arId}`" :label="item.admRegName"></el-option>
                                </el-select>
                            </el-form-item>
                    ...
                    <div slot="footer" class="dialogFoot">
                        <el-button @click="dbnameClose('dbList')" size="mini">取 消</el-button>
                        <el-button type="primary" @click="dbnameSave('dbList')" size="mini">确 定</el-button>
                    </div>
                </el-dialog>
                ...
                <el-dialog
                    ...
                    >
                        ...
                        <el-form-item label="省市">
                            <el-select
                            v-model="dbList.province"
                            placeholder="全国"
                            size="mini"
                            class="dishi"
                            clearable
                            @change="dbselectValueTable"
                            style="width: 30%"
                            >
                            <el-option
                                v-for="item in city"
                            ...
                            @change="selectCityValue"
                            >
                            <el-option
                                v-for="item in proCity"
                                :key="item.arId"
                                :value="item.arId"
                                :label="item.admRegName"
                            ></el-option>
                        ...
                        <el-form-item label="基站类型">
                            ...
                            <el-upload
                                action="''"
                                :auto-upload='true'
                                :show-file-list='uploadFlag'
                                :before-upload="beforeUpload"
                                :on-remove='excelDelete'
                                :before-remove="excelDelete"
                                :http-request="uploadExcel"
                                :limit='1'
                                :disabled="typeSelect == 0? true: false">
                                <el-button style="margin-left: 10px" type="primary" :disabled="typeSelect == 0? true: false"  size="small">上传规划工参</el-button>
                            </el-upload>
                        </el-form-item>
                        </div>
                    </el-form>
                    <div slot="footer" class="dialogFoot">
                        <el-button v-show='!uploadType' @click="errShow" size="mini">查看报错信息</el-button>
                        <el-button @click="modalhide" size="mini"
                        >取 消</el-button
                        >
                ...
                <div class="right" id="right">
                    <div class="rightTop">
                        ...
                        <el-dropdown v-if="tabType == '2D'" class="huizhi">
                            <el-button size="mini" style="width: 80px" type="info" plain v-has="'operation'">操作</el-button>
                            <el-dropdown-menu slot="dropdown">
                                <el-dropdown-item>
                                    <el-button type="info" plain size="mini" @click="invokeHtmlDraw">绘制</el-button>
                                </el-dropdown-item>
                                <el-dropdown-item>
                                    <el-button size="mini" type="info" plain @click="newPro">新建</el-button>
                                </el-dropdown-item>
                                <el-dropdown-item>
                                    <el-button size="mini" type="info" plain @click="uploadDialog">导入</el-button>
                                </el-dropdown-item>
                                <el-dropdown-item>
                                    <el-button type="info" plain size="mini" @click="uploadOut">导出</el-button>
                                </el-dropdown-item>
                            </el-dropdown-menu>
                        </el-dropdown>
                        ...
                        <el-checkbox class="zzhi" border size="mini" v-model="zhanzhi" @change="zzChange" v-if="tabType == '2D'" v-has="'stationAdress'">站址</el-checkbox>
        ...
        <iframe name="ce" id="ce" style="display: none"></iframe>
    </div>
</template>
<script>
...
                buildingAdujst: null,// 建筑物楼层设置
    ...
    created() {
        ...
        this.$axios({
            ...
            this.city = data.data //赋值省数据
            localStorage.setItem('city', JSON.stringify(this.city)) //将省列表进行缓存
        })
        this.$axios({
            method: 'get',
            url: this.httpUrl + '/radial/getAntennaNameList'
        }).then(data => {
            this.rayList = data.data
        })
        //状态下拉接口
        this.$axios({
            method: 'get',
            url: this.httpUrl + '/taskState'
        }).then(res => {
            this.statusList = res.data.list //状态列表赋值
        })
        //制式下拉接口
        this.$axios({
            method: 'get',
            url: this.httpUrl + '/standardSystem?parentType=0'
        }).then(data => {
            this.findStandardSystemList = data.data.list //制式列表赋值
        })
        //指标下拉接口
        this.$axios({
            method: 'get',
        ...
        this.$axios({
            method: 'get',
            url: this.httpUrl + '/radial/getLiShuttleModelListNoPage'
        }).then(res => {
            this.modelList = res.data
            //仿真模型
            this.$axios({
                method: 'get',
                url: this.httpUrl + '/simulationModel'
            }).then(res => {
                this.simulationModelList = res.data // 仿真模型列表赋值
                let modelShuList = []
                modelShuList = this.simulationModelList[0]
                let targetObject = {
                    ...modelShuList,
                    modelId: -1,
                    modelName: 'DTN-SPM'
                }
                this.modelList.unshift(targetObject)
            })
        })
        //左侧列表接口
        this.fetch()
    ...
    mounted() {
        ...
        // this.$axios({
        //     url: 'http://api.tianditu.gov.cn/search?postStr={"keyWord":"昆明","level":"11","mapBound":"116.04577,39.70307,116.77361,40.09583","queryType":"1","count":"20","start":"0"}&type=query&tk=58f322d5b366cbe267d21bd78b4a5351',
            
        //     // ajax('http://api.tianditu.gov.cn/search', (err, json_arr) => {
    ...
    methods: {
        ...
        /**
         * @description 头部网络类型切换
         * @author xiaowenjun
         * @since 20/05-07
         */
        topNetChange(item) {
            console.log('topchange====>', item)
            if (item == 1) {
                this.normArray = [
                    {
                        norm: '指标1',
                ...
                this.lastLongTitle = '满足SS-RSRP覆盖率且SS-SINR达标率的百分比'
                this.lastRSRPArray = [
                    {
                        title: 'SS-RSRP',
                        sign: '大于',
                ...
                this.lastLongTitle = '满足RSRP覆盖率且SINR达标率的百分比'
                this.lastRSRPArray = [
                    {
                        title: 'RSRP',
                        sign: '大于',
        ...
        // 导出—— 楼宇画像结果
        buildingPortraitResults() {
            // ids是区域ID
            let ids = this.multipleSelection         
            if (ids == [] || ids.split(",").length != 1) {
                this.$message({
                    message: '请选择区域且只能勾选一个区域',
                    type: 'warning',
                    center: true
                })
                return
            ...
            if(this.taskIds.length == 1){
                let baseUrl = this.imgUrl // 格式：http://218.205.192.26:8070
                //  todo 如果taskIds 即jobid对应路径下没有文件，导致无法提示。
                // 待优化：（1）文件名可加区域名，便于查看；（2）多文件、且文件较大时，需要改进设置 延迟触发事件和 下载删除时间
                let fileUrlList = []
                let coveragePercentUlr = baseUrl+'/Result/'+this.taskIds[0]+'/coveragePercent.csv'
                let buildingAbstracttUlr = baseUrl+'/Result/'+this.taskIds[0]+'/buildingAbstract.csv'
                let buildingFloorScenario8Ulr = baseUrl+'/Result/'+this.taskIds[0]+'/buildingFloorScenario8.csv'
                fileUrlList.push(coveragePercentUlr)
                fileUrlList.push(buildingAbstracttUlr)
                fileUrlList.push(buildingFloorScenario8Ulr)
                // 下载静态文件；
                this.downLoadAllLoad(fileUrlList)                               
            }else{
                this.$message({
                    message: '请选择且只能勾选一个区域！',
                    type: 'warning',
                    center: true
                })
                return
        ...
        // 下载静态文件；
        downLoadAllLoad(fileUrlList) { 
          for(let i=0;i<fileUrlList.length;i++){
            const iframe = document.createElement("iframe");
            iframe.style.display = "none";  
            iframe.style.height = 0; 
            iframe.src = fileUrlList[i]; 
        ...
        //编辑弹框
        dialogNameVisible(index, row) {
            console.log('editModal=====>', index, row)
            this.nameVisilble = true
            this.dataList = Object.assign({}, row)
            this.p = row.province
            this.c = row.city
            if (row.province) {
                this.$axios({
                    method: 'get',
                    url: this.httpUrl + `/adminiRegionCode?parentId=${row.province}`
                }).then(data => {
                    this.proCity = data.data
                })
            }
            if(row.city) {
                this.$axios({
                    method: 'get',
                    url: this.httpUrl + `/adminiRegionCode?parentId=${row.city}`
                }).then(data => {
                    this.cityArea = data.data
                })
            }
        },
        //编辑接口
        nameSave(dataList) {
            const praram = {
                polygonName: this.dataList.polygonName,
                polygonType: this.dataList.polygonType,
                polygonId: this.dataList.rpolygonId,
                polygonCode: this.dataList.polygonCode,
                province: this.dataList.province,
                city: this.dataList.city,
                townCode: this.dataList.townCode,
                menuType: '0',
                taskStatus: this.dataList.taskStatus,
                taskType: this.dataList.cellType == '4G'? 'LTE': 'NR'
            }
            this.$refs[dataList].validate(valid => {
                if (valid) {
                    this.$axios
                        .patch(this.httpUrl + '/tasks', praram)
                        .then(data => {
                            if (data.status === 200) {
                                this.nameVisilble = false //编辑成功关闭弹框
                                this.$message({
                                    type: 'success',
                                    message: '编辑成功!', //提示编辑成功
                                    center: true,
                                ...
                                setTimeout(() => {
                                    this.fetch() //编辑成功刷新区域列表
                                }, 2000)
                            }
                        })
                        .catch(err => {
                            this.nameVisilble = false //编辑失败关闭弹框
                            this.$message({
                                type: 'error',
                                message: err.response.data.error, //提示失败信息
                                center: true,
                                duration: 30000,
                                showClose: true
                            })
                            this.fetch() //编辑失败刷新区域列表
                        })
                }
            })
        },
        //保存新建多边形
        dbnameSave(dbList) {
            let roleCityList = sessionStorage.getItem('citycode').split(',')
            let roleType = sessionStorage.getItem('roleid')
            
            // let proBox = []
            // for(let i in roleCityList) {
            //     proBox.push(roleCityList[i].substring(0,2))
            // }
            // roleCityList.map(item => item.substring(0,2))
            // console.log('proBox====>', roleCityList)
            console.log('roleCItyy====>', roleCityList, this.dbList.city, 'roleiD====>', roleType)
            // if(roleType !== 1 || roleType !== 4) {
            //     var goOn = false
            //     for(let i in roleCityList) {
            //         if(roleCityList[i] == this.dbList.city) {
            //             goOn = true
            //         }
            //     }
            //     if(!goOn){
            ...
            const praram = {
                name: this.dbList.polygonName, //多边形name
                province: this.dbList.province, // 多边形省
                city: this.dbList.city, //多边形市
                townCode: this.dbList.cityArea,
                coordinates: this.coordinates, // 多边形经纬度坐标
                menuType: 0, //页面类型 重点区域监控
                cellType: this.cellType == '4G'? 'LTE': 'NR',
                taskType: this.cellType == '4G'? 'LTE': 'NR'
            }
            //判断省市不能为空
            if (this.dbList.city == '') {
                this.$message({
                ...
                this.$refs[dbList].validate(valid => {
                    if (valid) {
                        this.$axios
                            .post(this.httpUrl + '/polygons', praram)
                            .then(data => {
                                if (data.status === 200) {
                                    this.centerDialogVisible = false //新建成功关闭弹框
                                    this.$message({
                                        type: 'success',
                                        message: '保存成功!', //提示成功信息
                                        center: true,
                            ...
                            .catch(res => {
                                this.centerDialogVisible = false // 失败关闭弹框
                                this.dbList = {
                                    polygonName: '',
                                    // province: '440000',
                                    province: '',
                                ...
                                window.frames['dtnzdMap'].cancel_polygon() //清除多变形
                                this.$message({
                                    type: 'error',
                                    message: '保存失败!', //提示错误信息
                                    center: true,
        ...
        dbnameClose() {
            ...
            window.frames['dtnzdMap'].cancel_polygon()
        },
        //选择省
        selectValue(arId) {
            this.areaShow = true
            for(let i in this.crownCity) {
                if(arId == this.crownCity[i]) {
                    console.log(this.crownCity[i], arId)
                    this.areaShow = false
                }
            }
            if (arId) {
                this.$axios({
                    method: 'get',
                    url: this.httpUrl + `/adminiRegionCode?parentId=${arId}`
                }).then(res => {
                    console.log('pro====>', res)
                    this.proCity = res.data
        ...
        //选择市区
        selectCityValue(arId) {
            console.log('city===>', arId)
            if (arId) {
                this.$axios({
                    method: 'get',
                    url: this.httpUrl + `/adminiRegionCode?parentId=${arId}`
        ...
        selectArea(test) {
            console.log('sellect==town==>', this.dbList, test)
        },
        //编辑选择省
        selectValueTable(val) {
            this.dataList.province = val
            if (val !== this.p) {
                this.proCity = []
                this.dataList.city = ''
                this.$axios({
        ...
        //编辑选择市
        selectValueCity(val) {
            this.dataList.city = val
        },

        // 新建多边形选择省
        dbselectValueTable(val) {
            console.log('newPro====>', val, this.city)
            this.dbList.province = val
            for(let i in this.city) {
                if(val == this.city[i].arId) {
                    this.dbList.provinceName = this.city[i].admRegName
                }
            }
            // let thCity = [110000, 120000, 310000, 500000, 810000, 820000]
            // for(let i in thCity) {
            //     if(val == thCity[i]) {
            //         this.isThCity = true
            //     }
        ...
        // 新建多边形选择市
        dbselectValueCity(val) {
            console.log('citySelect===>', val, this.proCity, this.city, this.cityArea)
            this.dbList.city = val
            for(let i in this.proCity) {
                if(val == this.proCity[i].arId) {
                    this.dbList.cityName = this.proCity[i].admRegName
                }
            }
        },
        ...
        fetch() {
            this.$axios({
                method: 'get',
                url: this.httpUrl + `/tasks?pageNo=${this.currentPage}&pageSize=20&taskStatus=${this.taskStateType}&province=${this.arId}&city=${this.arId1}&polygonName=${this.input}&menuType=${this.menuType}`
            }).then(data => {
                ...
                this.$axios({
                    ...
                    this.tableData.map((item, i) => {
                        ...
                        this.alltaIDArray.push(item.taId)
                    })
                    this.alltaIDArray = this.alltaIDArray.join(',')

                    this.tabData = this.tableData

                    let arr1 = []
                    arr1 = eval(localStorage.getItem('arr'))
                    if (arr1) {
                        let praram1 = arr1.map(item => item.rpolygonId).toString()
                        //判断页面刷新，跳转页面 区域列表状态定时器开启关闭
                        this.timer = setInterval(() => {
                            this.$axios({
                                method: 'get',
                                url: this.httpUrl + `/tasks/getTaskStatus?polygonIds=${praram1}`
                            }).then(res => {
                                arr1 = res.data
                                ...
                                let st = arr1.map(item => item && item.taskStatus)
                                let newSt = [...new Set(st)]

                                let ft = newSt.filter(item => item !== '1').toString()
                                // let ft2 = newSt.filter(item => item == "0").toString();
                                if (ft) {
                                    arr1 = []
        ...
        uploadExcel (item) {
            ...
            this.$axios({
                method: 'post',
                url: this.httpUrl + '/stations/excel/yunnan/',
                data: form
            }).then(res => {
                ...
                if(res.data.code == 200) {
                    ...
                    for(let i = 0; i < res.data.error.length; i++) {
                        ...
                        cont.id = `序号${res.data.error[i].orderId}`
                        for(let j = 0; j < res.data.error[i].erreor.length; j++) {
                            let childCont = {
                                id: '',
                                filedName: null,
                                erroeInfo: null,
                            }
                            childCont.filedName = res.data.error[i].erreor[j].filedName
                            childCont.erroeInfo = res.data.error[i].erreor[j].erroeInfo
                            cont.children.push(childCont)
                        }
                        // cont.children[i] = childCont
                        this.excelError.push(cont)
                    }
                    this.excelError = this.excelError
                    console.log(this.excelError)
                    // this.excelError = res.data
                }
        ...
        searchLatLngPosition() {
            // 地图搜索
            let latlng = this.searchtext
            // 参数校验后使用，格式要求：23.121117817286503,113.32121399999998
            let lat = latlng.split(',')[0]
            let lng = latlng.split(',')[1]
            if (isNaN(parseFloat(lat)) || isNaN(parseFloat(lng))) {
                this.$message({
                    message: '请输入正确经纬度格式：lat,lng，示例：23.1211,113.32121',
                    type: 'warning',
                    center: true,
                    duration: 30000,
                    showClose: true
            ...
            window.frames['dtnzdMap'].search_position(latlng) //给gis search_position方法进行传参
        },
        //新建任务单弹框
        newPro() {
            this.modalShow = true
        ...
        selType(e) {
            this.typeSelect = e
            console.log('seltype====>', e)
        },

        submit() {
            console.log('submit===>', this.dbList, this.cellType == '4G'?'LTE': 'NR')
            let roleCityList = sessionStorage.getItem('citycode').split(',')
            // let goOn = false
            // for(let i in roleCityList) {
            //     if(roleCityList[i] == this.dbList.city) {
            ...
            if(this.dbList.polygonName == '') {
                ...
                return
            }else if (this.dbList.city == '' || this.dbList.province == '') {
                this.$message({
                    type: 'warning',
                    message: '必须选省、市、辖区!',
                    center: true,
                ...
                return
            }else if(this.excelError.length !== 0) {
                this.$message({
                    type: 'warning',
                    message: '上传工参有误，请查看工参报错信息',
                    center: true,
                    duration: 3000,
                    showClose: true
                })
            ...
            this.$axios({
                method: 'post',
                url: this.httpUrl + '/stations/yunnan/savaPolygon',
                data: {
                        fileName: this.uploadName,
                        simulationType: this.typeSelect,
                        regionDto: {
                            code: 0,
                            name: this.dbList.polygonName,
                            province: this.dbList.province,
                            townCode: this.dbList.cityArea,
                            city: this.dbList.city,
                            cityCode: this.dbList.city,
                            menuType: 0,
                            polygonType: this.typeSelect,
                            cellType: this.cellType == '4G'?'LTE': 'NR',
                            taskType: this.cellType == '4G'?'LTE': 'NR'
                        }
                }
        ...
        changeEditCell(e) {
            this.dataList.cellType = ''
            this.dataList.cellType = e
        },

        //搜索处制式改变
        searchZsValue(e) {
            this.searchZsType = this.searchZsType
        },

        excelDelete(eee) {
            console.log('delete===>', eee)
            this.excelError = []
            this.uploadType = true
        },
