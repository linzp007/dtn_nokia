...
; (function (window, document, undefined) {
	...
	var Dialog = function (config) {
		...
		if (typeof _follow === 'string' || _follow.tagName) {
			_follow = {
				'target': _follow.tagName ? _follow : document.getElementById(_follow),
				'placement': 'bottom'
			};
		}
		else if (typeof _follow === 'object' && _follow['target']) {
			_follow = {
				'target': typeof _follow['target'] === 'string'
		...
		content: function (_content) {
			...
			// HTMLElement
				...
				this._elemBack = function () {
					...
					_content.style.display = "none";//display;
					_this.elemBack = null;
				};
	...
	Dialog._tmpl = '<div class="idlg-outer" style="position:relative;">{rs_tmpl}'
		...
		+ '<div class="idlg-header" style="margin-left:57px"><div class="idlg-title"></div></div>'
		+ '<div class="idlg-main" style="display:flex;justify-content:center;margin-left:-10px;"></div>'
		+ '<div class="idlg-footer" style="display:none;"><div class="idlg-buttons" style="width:100%;display:flex;justify-content:center;margin-left:-7px;"></div></div>'
		//+	'</div>'
